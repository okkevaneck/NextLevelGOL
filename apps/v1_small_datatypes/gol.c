...
Added optimizations compared to the reference code:
 - Changed the world->cells datatype from int to uint8_t
...
static uint8_t **alloc_2d_int_array(int nrows, int ncolumns) {
    uint8_t **array;
    int i;
     ...
     * across dimensions.
     */
    array = malloc(nrows * sizeof(uint8_t *));
    if (array == NULL) {
        fprintf(stderr, "out of memory\n");
        exit(1);
    }

    array[0] = malloc(nrows * ncolumns * sizeof(uint8_t));
    if (array[0] == NULL) {
        fprintf(stderr, "out of memory\n");
        exit(1);
    }
